...
// PublicSagecityAPI provides an API to access Sagecity related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicSagecityAPI struct {
	b Backend
}

// NewPublicSagecityAPI creates a new Sagecity protocol API.
func NewPublicSagecityAPI(b Backend) *PublicSagecityAPI {
	return &PublicSagecityAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicSagecityAPI) GasPrice(ctx context.Context) (*big.Int, error) {
	return s.b.SuggestPrice(ctx)
}

// ProtocolVersion returns the current Sagecity protocol version this node supports
func (s *PublicSagecityAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicSagecityAPI) Syncing() (interface{}, error) {
...
//   keccak256("\x19Sagecity Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19Sagecity Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an Sagecity ECDSA signature for:
// keccack256("\x19Sagecity Signed Message:\n" + len(message) + message))
//
...
// the address of:
// hash = keccak256("\x19Sagecity Signed Message:\n"${message length}${message})
...
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	...
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid Sagecity signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the Sagecity blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Sagecity blockchain API.
...
// PublicDebugAPI is the collection of Sagecity APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of Sagecity APIs exposed over the private
// debugging endpoint.
...
// Version returns the current sagecity protocol version.
