...
// nodeDockerfile is the Dockerfile required to run an Sagecity node.
var nodeDockerfile = `
FROM sagecity/client-go:latest
...
RUN \
  echo 'geth init /genesis.json' > geth.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.sagecity/keystore/ && cp /signer.json /root/.sagecity/keystore/' >> geth.sh && \{{end}}
...
// an Sagecity node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
...
// deployNode deploys a new Sagecity node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.sagecity"],
