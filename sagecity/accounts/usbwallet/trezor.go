...
import (
	...
	"github.com/SagecityCore/sagecity/accounts/usbwallet/internal/trezor"
...
// or whether the Sagecity app was not started on it.
...
// Sagecity address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.SagecityAddress)
	if _, err := w.trezorExchange(&trezor.SagecityGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.SagecityTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.SagecityTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
