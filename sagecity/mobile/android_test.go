...
import (
	...
	"github.com/SagecityCore/sagecity/internal/build"
)
...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.sagecity", new NodeConfig());
			node.start();
			...
			SagecityClient ec = node.getSagecityClient();
	...
	// signatures too. Regression test for sagecity issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/sagecity/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.sagecity", "github.com/SagecityCore/sagecity/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "sagecity", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
